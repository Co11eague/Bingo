  ...
  */
  ...
  public String getCardNumbers() {
    ...
    for (int i = 0; i < getNumberOfRows(); i++) {
      for (int i1 = 0; i1 < getNumberOfColumns(); i1++) {
        sb.append(numbers[i][i1]);
        if (i1 + 1 != getNumberOfColumns()) sb.append(Defaults.getNumberSeparator());
      }
  ...
  public void setCardNumbers(String[] numbersAsStrings) {
    ...
    for (int i = 0; i < getNumberOfRows(); i++){
      if (getNumberOfColumns() >= 0)
        System.arraycopy(numbersList, i * getNumberOfColumns(), numbers[i], 0, getNumberOfColumns());
    }
  ...
  public void markNumber(int number) {
    ...
    for (int i = 0; i < numberOfRows; i++) {
      for (int i1 = 0; i1 < numberOfColumns; i1++) {
        if (number == numbers[i][i1]){
          System.out.println("Marked off "+number);
          marked = true;
          markedOff[i][i1] = true;
          //System.out.println(numbers[i][i1]);
        }
  ...
  public boolean isWinner() {
    ...
    for (int i = 0; i < numberOfRows; i++) {
      for (int i1 = 0; i1 < numberOfColumns; i1++) {
        if (Boolean.FALSE.equals(markedOff[i][i1])) return false;
